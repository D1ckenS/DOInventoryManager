...
namespace DOInventoryManager.Views
{
    public partial class BackupManagementView : UserControl
    {
        ...
        public BackupManagementView()
        {
            ...
            _bulkDataService = new BulkDataService();
            LoadBackupData();
...
Ready to select your file?";
        ...
        #region History Tab - Data Loading
        ...
        private async Task LoadHistoryDataAsync()
        {
            try
            {
                ...
                // Load vessels for filters
                _vessels = await context.Vessels.OrderBy(v => v.Name).ToListAsync();
                var allVesselsOption = new Vessel { Id = 0, Name = "All Vessels" };
                var vesselOptions = new List<Vessel> { allVesselsOption };
                vesselOptions.AddRange(_vessels);

                PurchaseVesselFilter.ItemsSource = vesselOptions;
                PurchaseVesselFilter.SelectedIndex = 0;
                ConsumptionVesselFilter.ItemsSource = vesselOptions;
                ConsumptionVesselFilter.SelectedIndex = 0;

                // Load suppliers for filters
                _suppliers = await context.Suppliers.OrderBy(s => s.Name).ToListAsync();
                var allSuppliersOption = new Supplier { Id = 0, Name = "All Suppliers" };
                var supplierOptions = new List<Supplier> { allSuppliersOption };
                supplierOptions.AddRange(_suppliers);

                PurchaseSupplierFilter.ItemsSource = supplierOptions;
                PurchaseSupplierFilter.SelectedIndex = 0;

                // Initialize data grids
                PurchaseHistoryGrid.ItemsSource = _purchaseItems;
                ConsumptionHistoryGrid.ItemsSource = _consumptionItems;
        ...
        private async Task RefreshPurchaseHistoryAsync()
        {
            try
            {
        ...
        private async Task RefreshConsumptionHistoryAsync()
        {
            try
            {
            ...
            {
                MessageBox.Show($"Error loading consumption history: {ex.Message}", "Error",
                              MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void ApplyPurchaseFilter_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var filter = new BulkDataService.PurchaseFilter
        ...
        private async void ClearPurchaseFilter_Click(object sender, RoutedEventArgs e)
        {
        ...
        private async void ApplyConsumptionFilter_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var filter = new BulkDataService.ConsumptionFilter
        ...
        private async void ClearConsumptionFilter_Click(object sender, RoutedEventArgs e)
        {
        ...
        private async void RefreshConsumptions_Click(object sender, RoutedEventArgs e)
        {
            await RefreshConsumptionHistoryAsync();
        }
        ...
        #region History Tab - Selection Management

        private void SelectAllPurchases_Checked(object sender, RoutedEventArgs e)
        {
        ...
        private void SelectAllPurchases_Unchecked(object sender, RoutedEventArgs e)
        {
            foreach (var item in _purchaseItems)
            {
        ...
        private void PurchaseSelection_Changed(object sender, RoutedEventArgs e)
        {
            UpdatePurchaseSelectionCount();
        }

        private void SelectAllConsumptions_Checked(object sender, RoutedEventArgs e)
        {
            foreach (var item in _consumptionItems)
            {
                item.IsSelected = true;
            }
            UpdateConsumptionSelectionCount();
        ...
        private void SelectAllConsumptions_Unchecked(object sender, RoutedEventArgs e)
        {
            foreach (var item in _consumptionItems)
            {
                item.IsSelected = false;
        ...
        private void ConsumptionSelection_Changed(object sender, RoutedEventArgs e)
        {
            UpdateConsumptionSelectionCount();
        }
        ...
        private void PurchaseHistoryGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            // Optional: Handle row selection if needed
        }

        private void ConsumptionHistoryGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            // Optional: Handle row selection if needed
        }
        ...
        #region History Tab - Bulk Deletion

        private async void DeleteSelectedPurchases_Click(object sender, RoutedEventArgs e)
        {
            var selectedPurchases = _purchaseItems.Where(p => p.IsSelected).ToList();
            if (!selectedPurchases.Any())
            {
                MessageBox.Show("No purchases selected for deletion.", "No Selection",
                              MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            var selectedIds = selectedPurchases.Select(p => p.Id).ToList();

            // Validate for allocations
            var allocatedCount = await _bulkDataService.ValidatePurchasesForDeletionAsync(selectedIds);
            if (allocatedCount == -1)
            {
                MessageBox.Show("Error validating purchases for deletion.", "Validation Error",
                              MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }

            // Show confirmation dialog
            var confirmationMessage = $"⚠️ BULK DELETION CONFIRMATION\n\n" +
                                    $"You are about to delete {selectedPurchases.Count} purchase records:\n\n" +
                                    $"📊 Selected Records: {selectedPurchases.Count}\n" +
                                    $"⚠️ Records with Allocations: {allocatedCount}\n\n";

            if (allocatedCount > 0)
            {
                confirmationMessage += $"🔄 This will also remove all related FIFO allocations.\n" +
                                     $"⚡ You may need to re-run FIFO allocation after deletion.\n\n";
            }

            confirmationMessage += $"💾 A backup will be created automatically before deletion.\n\n" +
                                 $"❗ This action cannot be undone.\n\n" +
                                 $"Do you want to proceed?";

            var result = MessageBox.Show(confirmationMessage, "Confirm Bulk Deletion",
                                       MessageBoxButton.YesNo, MessageBoxImage.Warning);
            ...
            try
            {
                DeleteSelectedPurchasesBtn.IsEnabled = false;
                DeleteSelectedPurchasesBtn.Content = "🔄 Deleting...";

                // Create backup before deletion
                await _backupService.CreateBackupAsync("BulkPurchaseDeletion");

                // Perform bulk deletion
                var deleteResult = await _bulkDataService.DeleteSelectedPurchasesAsync(selectedIds);

                if (deleteResult.Success)
                {
                    MessageBox.Show($"✅ Bulk deletion completed successfully!\n\n" +
                                  $"📊 Deleted: {deleteResult.DeletedCount} purchases\n" +
                                  $"❌ Failed: {deleteResult.FailedCount} purchases\n\n" +
                                  $"💡 Consider running FIFO allocation to update remaining data.",
                ...
                {
                    MessageBox.Show($"❌ Bulk deletion failed:\n\n{deleteResult.Message}\n\n" +
                                  $"Details:\n{string.Join("\n", deleteResult.Errors)}",
            ...
            {
                MessageBox.Show($"Error during bulk deletion: {ex.Message}", "Error",
                              MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
                DeleteSelectedPurchasesBtn.IsEnabled = true;
                DeleteSelectedPurchasesBtn.Content = "🗑️ Delete Selected";
            }
        }

        private async void DeleteSelectedConsumptions_Click(object sender, RoutedEventArgs e)
        {
            var selectedConsumptions = _consumptionItems.Where(c => c.IsSelected).ToList();
            if (!selectedConsumptions.Any())
            {
                MessageBox.Show("No consumptions selected for deletion.", "No Selection",
                              MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            var selectedIds = selectedConsumptions.Select(c => c.Id).ToList();

            // Validate for allocations
            var allocatedCount = await _bulkDataService.ValidateConsumptionsForDeletionAsync(selectedIds);
            if (allocatedCount == -1)
            {
                MessageBox.Show("Error validating consumptions for deletion.", "Validation Error",
                              MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            // Show confirmation dialog
            var confirmationMessage = $"⚠️ BULK DELETION CONFIRMATION\n\n" +
                                    $"You are about to delete {selectedConsumptions.Count} consumption records:\n\n" +
                                    $"📊 Selected Records: {selectedConsumptions.Count}\n" +
                                    $"⚠️ Records with Allocations: {allocatedCount}\n\n";
            ...
            try
            {
                DeleteSelectedConsumptionsBtn.IsEnabled = false;
                DeleteSelectedConsumptionsBtn.Content = "🔄 Deleting...";

                // Create backup before deletion
                await _backupService.CreateBackupAsync("BulkConsumptionDeletion");

                // Perform bulk deletion
                var deleteResult = await _bulkDataService.DeleteSelectedConsumptionsAsync(selectedIds);

                if (deleteResult.Success)
                {
                    MessageBox.Show($"✅ Bulk deletion completed successfully!\n\n" +
                                  $"📊 Deleted: {deleteResult.DeletedCount} consumptions\n" +
                                  $"❌ Failed: {deleteResult.FailedCount} consumptions\n\n" +
                                  $"💡 Consider running FIFO allocation to update remaining data.",
                                  "Deletion Completed", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            {
                DeleteSelectedConsumptionsBtn.IsEnabled = true;
                DeleteSelectedConsumptionsBtn.Content = "🗑️ Delete Selected";
            }
        }
